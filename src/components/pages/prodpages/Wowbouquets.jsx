...
function Wowbouquets() {
  ...
  const { wowbouquets } = useSelector(x => x);

  const [filtered, setFiltered] = useState(wowbouquets);


  // const [wowbouquets, setWowbouquets] = useState([])

  // useEffect(() => {
  //   fetch('http://localhost:7000/wowbouquets')
  //     .then(res => res.json())
  //     .then(data => setWowbouquets(data))
  // }, [])
  ...
  const addToFav = (id) => {
    let item = wowbouquets.find((a) => a.id === id);
  ...
  const addToCart = (id) => {
    let prod = wowbouquets.find((a) => a.id === id);
    let newProd = [...tocart];
                  ...
                  <div className="one-item" id={a.name.replace(/\s/g, "")} key={i}>
                    ...
                    >
                      <SwiperSlide className='item-slider'>
                        <img src={a.image1} alt="wowbouquet" />
                      </SwiperSlide>
                      <SwiperSlide className='item-slider'>
                        <img src={a.image2} alt="wowbouquet" />
                      </SwiperSlide>
                    </Swiper>
