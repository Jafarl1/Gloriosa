...
function Cardspage() {
  ...
  const { cardspage } = useSelector(x => x);
  ...
  const addToFav = (id) => {
    let item = cardspage.find((a) => a.id === id);
          ...
          <div className="all-items">
            {
              filtered.length > 0 ?
                filtered.map((a, i) => (
                  <div className="one-item" id={a.name.replace(/\s/g, "").replace(/ə/gi, 'e')} key={i}>
                    <Swiper
                      scrollbar={true}
                      modules={[Scrollbar]}
